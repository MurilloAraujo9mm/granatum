...
function url(string $path = null): string
{
    ...
    if ($path) {
        return CONF_URL_DEPLOY . "/" . ($path[0] == "/" ? substr($path, 1) : $path);
    }

    return CONF_URL_DEPLOY;
}
...
function theme(string $path = null, string $theme = CONF_VIEW_THEME): string
{
    ...
    if ($path) {
        return CONF_URL_DEPLOY . "/themes/{$theme}/" . ($path[0] == "/" ? mb_substr($path, 1) : $path);
    }

    return CONF_URL_DEPLOY . "/themes/{$theme}";
}
